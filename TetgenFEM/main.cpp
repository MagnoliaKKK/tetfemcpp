...
int main() {
	...
	char args[] = "pq1.414a1000";  // pq1.414a0.1 minratio 1/ mindihedral -q maxvolume -a switches='pq1.1/15a0.003' "pq1.1/15a0.0005 pq1.15a0.0001"
...
#pragma omp parallel for
		...
		if (glfwGetKey(window, GLFW_KEY_C) == GLFW_PRESS) {//是否开启保存点坐标
			std::ofstream file("DeformResultLocalFEM8000_0.49.txt", std::ios::out | std::ios::trunc);
			if (!file.is_open()) {
