...
float poisson = 0.49;
float density = 1000;
int groupNum, groupNumX = 10, groupNumY = 5, groupNumZ =6;//Object类和颜色都写死了 不能超出class Object {纴E淖槭?
int wKey = 0;
...
#pragma omp parallel for
	for (int i = 0; i < object.groupNum; ++i) {
		...
		object.groups[i].calLocalPos(); // 计算初始位置觼E跏贾匦牡牟?
	...
	while (!glfwWindowShouldClose(window)) {
		...
		for (int groupIdx = 0; groupIdx < groupNum; ++groupIdx) { //写点的眮E牛?
			//Group& group = object.getGroup(groupIdx);
		...
		nbFrames++;
		if (currentTime - lastTime >= 1.0) { // 如果过去了至少1脕E
			printf("%d frames/sec\n", nbFrames);
			nbFrames = 0;
			lastTime += 1.0;
		}
